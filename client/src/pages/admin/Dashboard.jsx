...
const AdminDashboard = () => {
  ...
  const fetchDashboardData = async () => {
    try {
      const [usersRes, leavesRes, policiesRes] = await Promise.all([
        api.get("/users"),
        api.get("/leaves/all"),
        api.get("/policies"),
      ])
    ...
    <Layout>
      <div className="space-y-6">
        <div>
          ...
          <p className="text-gray-600 mt-1">Here's what's happening with your organization</p>
        </div>

        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          <StatCard icon={Users} label="Total Users" value={stats.totalUsers} color="bg-blue-600" />
          <StatCard icon={TrendingUp} label="Active Users" value={stats.activeUsers} color="bg-green-600" />
          <StatCard icon={Clock} label="Pending Leaves" value={stats.pendingLeaves} color="bg-yellow-500" />
          <StatCard icon={CheckCircle} label="Approved Leaves" value={stats.approvedLeaves} color="bg-emerald-600" />
          <StatCard icon={XCircle} label="Rejected Leaves" value={stats.rejectedLeaves} color="bg-red-500" />
          <StatCard icon={FileText} label="Leave Policies" value={stats.totalPolicies} color="bg-indigo-600" />
        </div>
